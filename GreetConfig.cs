...
namespace GreetPlugin
{
    public class Config
    {
        [JsonProperty("Greet Cooldown")]
        public static int ShakeCooldown { get; set; } = 30;
        public static string[] RankList { get; set; } = { "User" };
        [JsonIgnore]
        private static readonly string Path = System.IO.Path.Combine(TShock.SavePath, "GreetPlugin.json");
        public static Config Create()
        {
            ...
            {
                TShock.Log.ConsoleError("[GreetPlugin] Failed to create new config file!");
            }
            return new Config();
        }
        public static Config Read(bool returnNull = false)
        {
            ...
            catch (JsonReaderException e)
            {
                ...
                TShock.Log.ConsoleError($"[GreetPlugin] Failed to load config!");
